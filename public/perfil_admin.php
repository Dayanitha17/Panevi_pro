...
<body>
            ...
            <ul class="admin-menu">
                ...
                <li>
                    <a href="../public/logout.php">
                        <span class="fa fa-sign-out-alt">     Cerrar Sesion</span>
                    </a>
                </li>
                <li>
                    <div class="switch">
    ...
    <section class="page-content">
        <section class="search-and-user">
            <form>
                    <h1 style="font-size: 35px; font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif;"><i>PRODUCTOS</i></h1>
                </button>
            </form>
            <div class="admin-profile">
                <span class="greeting"><h3><b><?php echo htmlspecialchars($_SESSION['nombre']); ?></b></h3></span>
        ...
        <hr>
        <br>
        <h1 style="font-size: 25px; color: green;">Crear Producto</h1>
...
<table id="datatable_users" class="table table-striped" style="text-align: center;">
    <thead>
        <tr>
            ...
            <th class="centered">Imagen del producto</th>
        </tr>
        ...
        <tr>
            ...
            <td><label for="file-upload" class="custom-file-upload">Subir Imagen</label>
            <input id="file-upload" type="file" name="imagen" /></td>
            <td><button  id="boton" class="btn btn-sm btn-primary">
                <span>Añadir producto</span></button></td>
        </tr>
    </thead>
</table>
<br>
...
if ($result->num_rows > 0) {
    ...
    echo "<thead><tr><th>ID</th><th>Nombre</th><th>Precio</th><th>Imagen</th><th>Acciones</th></tr></thead>";
    echo "<tbody>";
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>
                <a href='?edit=" . $row["id_producto"] . "' class='btn btn-sm btn-primary'><i class='fa fa-pencil'></i> Editar</a>
                <button class='btn btn-sm btn-danger' onclick='eliminarProducto(" . $row["id_producto"] . ")'><i class='fa fa-trash'></i> Eliminar</button>
              </td>";
        echo "</tr>";
    }
    echo "</tbody>";
...
if (isset($_POST['update'])) {
    ...
    $nombre = $_POST['nombre'];
    $precio = $_POST['precio'];
    $imagen = $_POST['imagen'];  // Ahora tomamos la ruta de la imagen directamente del input de texto

    // Actualizar la base de datos
    $sql = "UPDATE productos SET nombre='$nombre', precio='$precio', imagen='$imagen' WHERE id_producto=$id_producto";

    if ($conexion->query($sql) === TRUE) {
        $mensaje = "Producto actualizado exitosamente";
        $tipo_mensaje = "exito";
    } else {
        $mensaje = "Error actualizando el producto: " . $conexion->error;
        $tipo_mensaje = "error";
    }
}
?>
...
<?php if (isset($_GET['edit'])): ?>
    <hr>
    <br>
    <h1 style="font-size: 25px; color: green;">Editar Producto</h1>
    <form method="post" action="">
        ...
        <tbody id="tableBody_users">
            <tr>
                <td><input style="width: 150px; margin: 5px; margin-right: 7px;" type="text" name="nombre" required value="<?php echo $row['nombre']; ?>"></td>
                <td><input style="width: 150px; margin: 5px; margin-right: 7px;" type="number" name="precio" required value="<?php echo $row['precio']; ?>"></td>
                <td><input style="width: 230px; margin: 5px; margin-right: 7px;" type="text" name="imagen" value="<?php echo $row['imagen']; ?>"></td>
                <td><button  name="update" id="boton" class="btn btn-sm btn-primary">
                    <span>Actualizar producto</span></button></td>
            </tr>
        </tbody>
        </table>
